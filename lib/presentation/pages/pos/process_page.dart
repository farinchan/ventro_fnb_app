...
import 'package:ventro_fnb_app/presentation/widgets/dialog_payment_success.dart';
import 'package:ventro_fnb_app/presentation/widgets/dialog_transaction_save.dart';
...
class _ProcessPageState extends State<ProcessPage> {
  ...
  void _handlePayCashTransaction() async {
    var stafOutletId = await LocalDatasource().getStaffOutletId();
    if (!mounted) return;
    if (_payAmountController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Masukkan nominal pembayaran')));
      return;
    }
    if (_changeAmount < 0) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Nominal pembayaran kurang')));
      return;
    }
    context.read<TransactionBloc>().add(
      TransactionPayCash(
      ...
      body: BlocListener<TransactionBloc, TransactionState>(
        listener: (context, state) {
          ...
          if (state is TransactionLoaded) {
            showDialog(
              context: context,
              builder: (context) => DialogPaymentSuccess(saleId: state.saleEntity.id),
            );
          }
          if (state is TransactionSaved) {
            showDialog(
              context: context,
              builder: (context) => DialogTransactionSave(saleId: state.saleEntity.id),
            );
          }
          if (state is TransactionError) {
            ...
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(SnackBar(content: Text(state.error.message ?? "Terjadi kesalahan: silahkan hubungi admin")));
          }
        },
        child: Row(
          children: [
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              onPressed: _selectedPaymentMethod == null ? null : _handlePayCashTransaction,
