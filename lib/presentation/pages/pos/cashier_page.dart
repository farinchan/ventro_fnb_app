...
class _CashierPageState extends State<CashierPage> {
  ...
  void _onSearchChanged() {
    context
        .read<CashierBloc>()
        .add(CashierSearchProducts(query: _searchController.text));
  }
  ...
  void _showVariantPicker(BuildContext context, ProductEntity product) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              ...variants.map(
                (v) {
                  final price = int.tryParse(v.price ?? '0') ?? 0;
                  return ListTile(
    ...
    return BlocBuilder<CashierBloc, CashierState>(
      builder: (context, state) {
        if (isMobile) {
          return Column(
            children: [
              Expanded(child: _buildProductsPanel(context, state)),
              SizedBox(height: 300, child: _buildCartPanel(context, state)),
            ],
          );
        }
        ...
        return Row(
          children: [
            Expanded(flex: 3, child: _buildProductsPanel(context, state)),
            const VerticalDivider(width: 1),
            SizedBox(width: 370, child: _buildCartPanel(context, state)),
          ],
        );
      },
    );
  ...
  //  Products Panel
  // ──────────────────────────────────────────────
  Widget _buildProductsPanel(BuildContext context, CashierState state) {
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              if (state.status == CashierStatus.loaded)
                AnimatedSwitcher(
                  duration: const Duration(milliseconds: 300),
                  child: Text(
                    '${state.filteredProducts.length} produk',
  ...
  Widget _buildProductsBody(BuildContext context, CashierState state) {
    final theme = Theme.of(context);
    ...
    switch (state.status) {
      ...
      case CashierStatus.loaded:
        ...
        return _buildGridView(context, products, theme);
    }
  }

  Widget _buildGridView(
      BuildContext context, List<ProductEntity> products, ThemeData theme) {
  ...
  Widget _buildProductCard(
      BuildContext context, ProductEntity product, ThemeData theme) {
    final firstPrice = int.tryParse(product.variants?.first.price ?? '0') ?? 0;
  ...
  //  Cart Panel
  // ──────────────────────────────────────────────
  Widget _buildCartPanel(BuildContext context, CashierState state) {
    final theme = Theme.of(context);
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () => context
                    .read<CashierBloc>()
                    .add(CashierRemoveFromCart(cartItemId: item.id)),
              ),
            ],
          ),
          const SizedBox(height: 8),
                ...
                child: Row(
                  ...
                  children: [
                    _qtyButton(
                      ...
                      onPressed: () => context
                          .read<CashierBloc>()
                          .add(CashierDecrementQty(cartItemId: item.id)),
                    ),
                    Padding(
                    ...
                    _qtyButton(
                      ...
                      onPressed: () => context
                          .read<CashierBloc>()
                          .add(CashierIncrementQty(cartItemId: item.id)),
                    ),
                  ],
                ),
              ),
